...
#define ISVISIBLE(C)            ((1ULL << C->tag_idx & C->mon->tags))
...
applyrules(Client *c)
{
	...
	if (ch.res_name)
		XFree(ch.res_name);
	if (tidx < 0 || tidx >= LENGTH(tags)) {
		c->tag_idx = c->mon->spawn_tag_idx;
	}
}
...
tag(const Arg *arg)
{
  ...
  uint64_t arg_tag = 1ULL << arg->ui & TAGMASK;
  if (!arg_tag) return;

  selmon->sel->tag_idx = arg->ui;
  focus(NULL);
  arrange(selmon);
}
...
view(const Arg *arg)
{
  ...
  uint64_t arg_tag = 1ULL << tag_idx & TAGMASK;
